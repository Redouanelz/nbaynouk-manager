<x-layout title="Vue d’ensemble">
    ...
    <div class="mt-8 grid gap-5 xl:grid-cols-[1.4fr_.6fr]">
        ...
        <section><div class="section-heading"><div><p class="eyebrow">Portefeuille</p><h2>Répartition</h2></div></div><div class="space-y-1">@foreach(\App\Enums\ProjectStatus::cases() as $status)@if(($statusCounts[$status->value] ?? 0)>0)<div class="flex items-center justify-between border-b border-border py-3 text-sm"><span>{{ $status->label() }}</span><span class="text-xl font-bold">{{ $statusCounts[$status->value] }}</span></div>@endif @endforeach</div></section>
    </div>
    <section class="mt-16"><div class="section-heading"><div><p class="eyebrow">Derniers dossiers</p><h2>Projets récents</h2></div><a href="{{ route('projects.index') }}">Tous les projets</a></div>
        <div class="table-wrap"><table><thead><tr><th>Projet</th><th>Client</th><th>Statut</th><th>Avancement</th><th>Montant</th><th>Payé</th><th>Reste</th><th>Échéance</th></tr></thead><tbody>@forelse($recentProjects as $project)<tr class="clickable-row" data-href="{{ route('projects.show',$project) }}"><td><strong>{{ $project->business->name }}</strong><span>{{ $project->code }}</span></td><td>{{ $project->business->client->name }}</td><td><x-badge :value="$project->status" /></td><td><strong>{{ $project->progress_percentage }}%</strong><x-progress-bar :value="$project->progress_percentage" compact/></td><td>{{ \App\Support\Money::format($project->amount) }}</td><td>{{ \App\Support\Money::format($project->total_paid) }}</td><td>{{ \App\Support\Money::format($project->remaining_amount) }}</td><td>{{ $project->next_payment_date?->translatedFormat('d M') ?? '—' }}</td></tr>@empty<tr><td colspan="8">Aucun projet récent.</td></tr>@endforelse</tbody></table></div>
    </section>
</x-layout>
